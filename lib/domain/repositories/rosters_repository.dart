...
class RostersRepository {
  Stream<List<Roster>> rosters() => isar.rosters.where().watch(fireImmediately: true);
  Stream<Roster?> sid(int id) => isar.txnSync(
        () => isar.rosters.watchObject(
          id,
          fireImmediately: true,
        ),
      );

  void setRoster(Roster roster) => isar.writeTxnSync(() => isar.rosters.putSync(roster));
